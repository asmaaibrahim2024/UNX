...
const BookMark = () => {
      ...
      try {
        ...
        handle = bookMarkWG.bookmarks.on("change", function (evt) {
          evt.added.forEach(function (e) {
            ...
            newBookmark &&
              saveBookmarkToDatabase(newBookmark).then(async () => {
  ...
  const saveBookmarkToDatabase = async (bookmark) => {
   ...
   await restHelper.postRequest(
        `${window.mgAppConfig.apiServer.DGDA_ViewerApiURL}BookMarks/AddBookmark`,
        bookmark
      );
    } catch (error) {
  ...
  const updateBookmarkInDatabase = async (bookmark) => {
    try {
       await restHelper.postRequest(
        `${window.mgAppConfig.apiServer.DGDA_ViewerApiURL}BookMarks/UpdateBookmark`,
        bookmark
      );
    } catch (error) {
  ...
  const fetchBookmarksFromDatabase = async () => {
    try {
      const response = await restHelper.getRequest(
        `${window.mgAppConfig.apiServer.DGDA_ViewerApiURL}BookMarks/GetAllBookmarks`
      );
    ...
    try {
      if (bookmarkId) {
        await restHelper.getRequest(
          `${window.mgAppConfig.apiServer.DGDA_ViewerApiURL}BookMarks/DeleteBookmark?bookmarkId=${bookmarkId}`
        );
      }
    } catch (error) {
